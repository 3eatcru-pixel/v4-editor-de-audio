...
export class AudioCoreEngine {
  ...
  constructor(options: {
    ...
    // Start listening autonomously via the EventBus tick events
    this.scheduler.startListening({
      getBpm: options.getBpm,
      getActiveNotes: () => this.getActiveNotes ? this.getActiveNotes() : [],
      getHeldKeys: () => this.getHeldKeys ? this.getHeldKeys() : [],
      getArpEnabled: () => this.arpEnabled,
      getArpDivision: () => this.arpDivision,
      getArpPattern: () => this.arpPattern,
      getNoteRepeatEnabled: () => this.noteRepeatEnabled,
      getNoteRepeatDivision: () => this.noteRepeatDivision
    });
    ...
    this.transport = new TransportEngine({
      ...
      onTick: (absoluteTicks, preciseTime) => {
        // Since the scheduler listens directly to the eventBus to schedule playheads,
        // we only trigger the custom tick hooks here. This fully decouples the sequencer timing code.
        if (this.onTickHook) {
